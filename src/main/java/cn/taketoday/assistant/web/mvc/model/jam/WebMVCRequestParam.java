...
public class WebMVCRequestParam extends JamBaseElement<PsiParameter> {
  ...
  private static final JamAnnotationMeta ANNOTATION_META = new JamAnnotationMeta(InfraMvcConstant.REQUEST_PARAM).addAttribute(VALUE_META).addAttribute(NAME_META).addAttribute(REQUIRED_META)
          .addAttribute(DEFAULT_VALUE_META);
  public static final JamMemberMeta<PsiParameter, WebMVCRequestParam> META = new JamParameterMeta<>(WebMVCRequestParam.class)
          .addAnnotation(ANNOTATION_META);

  public WebMVCRequestParam(PsiElementRef<?> ref) {
    super(ref);
  }
