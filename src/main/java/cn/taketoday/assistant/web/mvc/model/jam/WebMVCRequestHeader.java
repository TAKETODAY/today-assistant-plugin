...
public class WebMVCRequestHeader extends JamBaseElement<PsiParameter> {
  private static final JamStringAttributeMeta.Single<String> VALUE_META = JamAttributeMeta.singleString(RequestMapping.VALUE_ATTRIBUTE, new KnownRequestHeadersJamConverter());
  private static final JamStringAttributeMeta.Single<String> NAME_META = JamAttributeMeta.singleString("name", new KnownRequestHeadersJamConverter());
  ...
  private static final JamAnnotationMeta ANNOTATION_META = new JamAnnotationMeta(InfraMvcConstant.REQUEST_HEADER).addAttribute(VALUE_META).addAttribute(NAME_META).addAttribute(REQUIRED_META)
          .addAttribute(DEFAULT_VALUE_META);
  public static final JamMemberMeta<PsiParameter, WebMVCRequestHeader> META = new JamParameterMeta<>(WebMVCRequestHeader.class)
          .addAnnotation(ANNOTATION_META);

  public WebMVCRequestHeader(PsiElementRef<?> ref) {
    super(ref);
  }
  ...
  private static class KnownRequestHeadersJamConverter extends JamConverter<String> {

    @Nullable
    public Object m83fromString(@Nullable String str, JamStringAttributeElement jamStringAttributeElement) {
    ...
    public PsiReference[] createReferences(JamStringAttributeElement<String> context, PsiLanguageInjectionHost injectionHost) {
      return HttpHeaderReference.forElement(injectionHost);
    }
  }
}
