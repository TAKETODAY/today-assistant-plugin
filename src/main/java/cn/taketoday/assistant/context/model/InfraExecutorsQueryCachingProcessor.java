...
abstract class InfraExecutorsQueryCachingProcessor<InParams extends ModelSearchParameters, OutParams extends BeanSearchParameters> extends InfraCachingProcessor<InParams> {
  protected abstract ExecutorsQuery<BeanPointer<?>, OutParams> createQuery(InParams inparams);

  public InfraExecutorsQueryCachingProcessor() {
    super(Conditions.alwaysTrue());
  }

  public InfraExecutorsQueryCachingProcessor(Condition<? super InParams> keyValidityCheck) {
    super(keyValidityCheck);
  }

  @Override
  protected Collection<BeanPointer<?>> findPointers(InParams parameters) {
    ExecutorsQuery<BeanPointer<?>, OutParams> query = createQuery(parameters);
