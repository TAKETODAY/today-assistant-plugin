...
public class ImportAutoConfigDependentModelsProvider extends AbstractAutoConfigDependentModelsProvider {
  ...
  protected List<PsiClass> getAutoConfigClasses(LocalAnnotationModel localAnnotationModel, ConditionalOnEvaluationContext sharedContext) {
    ImportAutoConfiguration importAutoConfiguration = ImportAutoConfiguration.META.getJamElement(localAnnotationModel.getConfig());
    return importAutoConfiguration == null ? Collections.emptyList() : AutoConfigClassCollector.collectConfigurationClasses(importAutoConfiguration, sharedContext);
  }

  @Override
  protected LocalModelDependencyType getModelDependencyType() {
    return LocalModelDependencyType.IMPORT;
  }
}
