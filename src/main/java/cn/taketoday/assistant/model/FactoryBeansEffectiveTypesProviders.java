...
public class FactoryBeansEffectiveTypesProviders extends BeanEffectiveTypeProvider {

  @Override
  public boolean processEffectiveTypes(CommonInfraBean bean, Processor<PsiType> processor) {
    PsiType beanType = bean.getBeanType();
    PsiClass beanClass = PsiTypesUtil.getPsiClass(beanType);
    if (beanClass != null && FactoryBeansManager.of().isFactoryBeanClass(beanClass)) {
      for (PsiType type : BeanCoreUtils.getFactoryBeanTypes(beanType, bean)) {
